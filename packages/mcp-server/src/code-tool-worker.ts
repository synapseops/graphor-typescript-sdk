...
import { Graphor, ClientOptions } from 'graphor';
...
function getTSDiagnostics(code: string): string[] {
  ...
  const codeWithImport = [
    ...
    functionSource.type === 'declaration' ?
      `async function run(${functionSource.client}: Graphor)`
    : `const run: (${functionSource.client}: Graphor) => Promise<unknown> =`,
    functionSource.code,
  ].join('\n');
