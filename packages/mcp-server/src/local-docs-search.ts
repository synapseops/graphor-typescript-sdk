...
const EMBEDDED_METHODS: MethodEntry[] = [
  ...
  },
  {
    ...
    markdown:
      "## ingest_file\n\n`client.sources.ingestFile(file: string, enrichment?: string, indexing?: string, method?: 'fast' | 'balanced' | 'accurate' | 'agentic' | 'auto'): { build_id: string; error?: string; success?: boolean; }`\n\n**post** `/sources/ingest-file`\n\nUpload a local file and schedule ingestion in the background.\n\nAccepts **`multipart/form-data`** with the file. Validates size (max 100 MB) and\nextension, stores the file, then schedules the full data-ingestion pipeline in the\nbackground. Returns immediately with a `build_id` to poll for status.\n\n**Parameters:**\n- **file** (`multipart/form-data`): The file to upload. Must include `Content-Length`\n  and have a supported extension (pdf, doc, docx, csv, txt, md, etc.).\n- **method** (`form`, optional): Partitioning strategy. One of: `fast`,\n  `balanced`, `accurate`, `vlm`, `agentic`, `auto`. Default when omitted.\n- **enrichment** (`form`, optional): `full` (default) or `none`. `none` skips\n  LLM page/section/document annotation for faster, cheaper parsing.\n- **indexing** (`form`, optional): `full` (default) or `none`. `none` skips\n  chunking/embedding/indexing — the source is parsed but not searchable via\n  ask/extract/retrieve. Check `GET /v2/sources/config` for availability.\n\n**Returns** `AsyncIngestResponse` with `build_id`. Use it to check processing status.\n\n### Parameters\n\n- `file: string`\n\n- `enrichment?: string`\n  LLM enrichment level: 'full' (default) or 'none' to skip page/section/document annotation for faster parsing.\n\n- `indexing?: string`\n  Retrieval indexing level: 'full' (default) or 'none' to skip chunking/embedding/indexing — the source is parsed but not searchable. Requires Temporal ingestion.\n\n- `method?: 'fast' | 'balanced' | 'accurate' | 'agentic' | 'auto'`\n  Public-facing partition method names for API v2.\n\nMaps to internal PartitionMethod as:\n- fast     → basic\n- balanced → hi_res\n- accurate → hi_res_ft\n- agentic  → graphorlm\n- auto     → auto\n\n### Returns\n\n- `{ build_id: string; error?: string; success?: boolean; }`\n\n  - `build_id: string`\n  - `error?: string`\n  - `success?: boolean`\n\n### Example\n\n```typescript\nimport Graphor from 'graphor';\n\nconst client = new Graphor();\n\nconst response = await client.sources.ingestFile({ file: fs.createReadStream('path/to/file') });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response: '{ build_id: string; error?: string; success?: boolean; }',
    markdown:
      "## ingest_url\n\n`client.sources.ingestURL(url: string, crawlUrls?: boolean, enrichment?: string, indexing?: string, method?: 'fast' | 'balanced' | 'accurate' | 'agentic' | 'auto'): { build_id: string; error?: string; success?: boolean; }`\n\n**post** `/sources/ingest-url`\n\nIngest a web page (or a set of crawled pages) as a source into the project's knowledge graph.\n\nUnlike the synchronous version, this endpoint schedules the ingestion in the background\nand returns immediately with a `processing` status. The source will be fully available\nonce background processing completes.\n\nIf the URL points directly to a downloadable file (detected via URL path extension or\nHTTP Content-Type), the file is first downloaded and uploaded to storage synchronously,\nthen the partition/embedding pipeline runs in the background.\n\n**Parameters (JSON body):**\n- **url** (str, required): The web page URL to ingest.\n- **crawlUrls** (bool, optional, default `false`): When `true`, the system will also\n  follow and ingest links found on the page. Ignored when the URL resolves to a file.\n- **method** (str, optional): The partitioning strategy to use.\n  One of: `fast`, `balanced`, `accurate`, `vlm`, `agentic`, `auto`. When omitted the system default is applied.\n\n**Returns** a `PublicSourceResponse` with `status: \"processing\"` immediately.\nPoll the source status endpoint using the returned `file_id` to track completion.\n\n**Error responses:**\n- `400` — Unsupported file type detected from a file URL.\n- `500` — Unexpected internal error during URL processing.\n\n### Parameters\n\n- `url: string`\n  The web page URL to ingest\n\n- `crawlUrls?: boolean`\n  When true, also follows and ingests links found on the page\n\n- `enrichment?: string`\n  LLM enrichment level. `full` (default) runs page/section and document annotation; `none` returns parsing results only and is faster, at the cost of weaker retrieval context.\n\n- `indexing?: string`\n  Retrieval indexing level. `full` (default) chunks, embeds and indexes the source; `none` skips it — the source will NOT be searchable via ask/extract/retrieve, though its parsed elements remain readable. Reversible by re-processing.\n\n- `method?: 'fast' | 'balanced' | 'accurate' | 'agentic' | 'auto'`\n  Public-facing partition method names for API v2.\n\nMaps to internal PartitionMethod as:\n- fast     → basic\n- balanced → hi_res\n- accurate → hi_res_ft\n- agentic  → graphorlm\n- auto     → auto\n\n### Returns\n\n- `{ build_id: string; error?: string; success?: boolean; }`\n\n  - `build_id: string`\n  - `error?: string`\n  - `success?: boolean`\n\n### Example\n\n```typescript\nimport Graphor from 'graphor';\n\nconst client = new Graphor();\n\nconst response = await client.sources.ingestURL({ url: 'https://example.com/blog/ai-trends-2025' });\n\nconsole.log(response);\n```",
    perLanguage: {
      typescript: {
  ...
  },
  {
    ...
    response:
      '{ file_name: string; file_size: number; file_source: string; file_type: string; message: string; project_id: string; project_name: string; status: string; file_id?: string; method?: string; searchable?: boolean; }[]',
    markdown:
