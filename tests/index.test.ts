...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['GRAPHOR_PRD_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: GraphorPrd) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['GRAPHOR_PRD_LOG'] = 'debug';
      const client = new GraphorPrd({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['GRAPHOR_PRD_LOG'] = 'not a log level';
      const client = new GraphorPrd({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'GRAPHOR_PRD_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new GraphorPrd({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['GRAPHOR_PRD_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new GraphorPrd({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['GRAPHOR_PRD_BASE_URL'] = 'https://example.com/from_env';
      const client = new GraphorPrd({ apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new GraphorPrd({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['GRAPHOR_PRD_BASE_URL'] = 'http://localhost:5000/env';
      const client = new GraphorPrd({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
