...
import { APIPromise } from 'graphor-typescript-project/core/api-promise';

import util from 'node:util';
import GraphorTypescriptProject from 'graphor-typescript-project';
import { APIUserAbortError } from 'graphor-typescript-project';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['GRAPHOR_TYPESCRIPT_PROJECT_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: GraphorTypescriptProject) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new GraphorTypescriptProject({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['GRAPHOR_TYPESCRIPT_PROJECT_LOG'] = 'debug';
      const client = new GraphorTypescriptProject({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['GRAPHOR_TYPESCRIPT_PROJECT_LOG'] = 'not a log level';
      const client = new GraphorTypescriptProject({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'GRAPHOR_TYPESCRIPT_PROJECT_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    ...
    afterEach(() => {
      process.env['GRAPHOR_TYPESCRIPT_PROJECT_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new GraphorTypescriptProject({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['GRAPHOR_TYPESCRIPT_PROJECT_BASE_URL'] = 'https://example.com/from_env';
      const client = new GraphorTypescriptProject({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['GRAPHOR_TYPESCRIPT_PROJECT_BASE_URL'] = ''; // empty
      const client = new GraphorTypescriptProject({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://graphorlm.com/api/public/v1');
    });

    test('blank env variable', () => {
      process.env['GRAPHOR_TYPESCRIPT_PROJECT_BASE_URL'] = '  '; // blank
      const client = new GraphorTypescriptProject({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://graphorlm.com/api/public/v1');
    });
    ...
    test('in request options overridden by env variable', () => {
      process.env['GRAPHOR_TYPESCRIPT_PROJECT_BASE_URL'] = 'http://localhost:5000/env';
      const client = new GraphorTypescriptProject({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new GraphorTypescriptProject({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new GraphorTypescriptProject({ apiKey: 'My API Key' });
  ...
  test('with environment variable arguments', () => {
    ...
    process.env['GRAPHOR_TYPESCRIPT_PROJECT_API_KEY'] = 'My API Key';
    const client = new GraphorTypescriptProject();
  ...
  test('with overridden environment variable arguments', () => {
    // set options via env var
    process.env['GRAPHOR_TYPESCRIPT_PROJECT_API_KEY'] = 'another My API Key';
    const client = new GraphorTypescriptProject({ apiKey: 'My API Key' });
    expect(client.apiKey).toBe('My API Key');
  });
});
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new GraphorTypescriptProject({ apiKey: 'My API Key', fetch: testFetch });
