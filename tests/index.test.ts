...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['GRAPHOR_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Graphor) => {
    ...
    test('default logLevel is warn', async () => {
      const client = new Graphor({ apiKey: 'My API Key' });
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      process.env['GRAPHOR_LOG'] = 'debug';
      const client = new Graphor({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      process.env['GRAPHOR_LOG'] = 'not a log level';
      const client = new Graphor({ logger: logger, apiKey: 'My API Key' });
      expect(client.logLevel).toBe('warn');
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'GRAPHOR_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Graphor({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['GRAPHOR_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Graphor({ baseURL: 'https://example.com', apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com');
    });

    test('env variable', () => {
      process.env['GRAPHOR_BASE_URL'] = 'https://example.com/from_env';
      const client = new Graphor({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('https://example.com/from_env');
    });

    test('empty env variable', () => {
      process.env['GRAPHOR_BASE_URL'] = ''; // empty
      const client = new Graphor({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('/api/public/v1');
    });

    test('blank env variable', () => {
      process.env['GRAPHOR_BASE_URL'] = '  '; // blank
      const client = new Graphor({ apiKey: 'My API Key' });
      expect(client.baseURL).toEqual('/api/public/v1');
    });
    ...
    test('in request options overridden by client options', () => {
      const client = new Graphor({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['GRAPHOR_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Graphor({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Graphor({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Graphor({ apiKey: 'My API Key' });
